...
The intention of having a gateway into the SDLC is to trap incorrect code, bugs, as early as possible,
thereby preventing incorrect code from passing from the local codebase to a shared codebase.
To get through the quality gateway code must satisfy several <<tests>>.
...
been already collected by performance tests and put as an NFR's thresholds/gates. Metrics collected during NFR gate tests are to be trended in time.
